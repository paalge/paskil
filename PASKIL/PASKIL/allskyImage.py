...
class allskyImage:
    ...
    def __init__(self, image, image_file, info):
        # This function in run when the class is instanciated. It sets up the
        # class attributes.
    ...
    def getInfo(self):
        ...
        copy = {'camera': self.__info['camera'].copy(), 'header': self.__info['header'].copy(
        ), 'processing': self.__info['processing'].copy(), 'exif': self.__info['exif'].copy()}
        return copy
    ...
    def absoluteCalibration(self, spectral_responsivity, exposure_time, const_factor=1.0):
        ...
        new_info['processing']['absoluteCalibration'] = const_factor / \
            float(spectral_responsivity * exposure_time * 1000)
    ...
    def addTimeStamp(self, format, colour="black", fontsize=20):
        ...
        try:
            time = datetime.datetime.strptime(
                self.__info['header']['Creation Time'], "%d %b %Y %H:%M:%S %Z")
        except ValueError:
            time = datetime.datetime.strptime(
        ...
        draw.text((x_position, new_image.size[
                  1] - text_height - 4), time_string, font=font, fill=colour)
    ...
    def alignNorth(self, north="geographic", orientation='NESW'):
        ...
        try:
            if new_info['processing']['alignNorth'].count('NESW') != 0:
                ...
                new_image = self.__image.rotate(-
                                                float(new_info['camera']['cam_rot']))
            elif new_info['processing']['alignNorth'].count('NWSE') != 0:
                # rotate anti-clockwise since the Image is in a NWSE
                # orientation
                new_image = self.__image.rotate(
                    float(new_info['camera']['cam_rot']))
            else:
                ...
                new_image = self.__image.rotate(-
                                                float(new_info['camera']['cam_rot']))
                new_info['processing']['alignNorth'] = new_info[
                    'processing']['alignNorth'] + "(NESW)"

        except KeyError:
            # the image hasn't been aligned North before, we assume it is in
            # NWSE orientation and change the info to reflect this
    ...
    def angle2dist(self, angle):
        ...
        if self.__info['camera']['lens_projection'] == 'equidistant':
            ...
            focal_length = float(
                self.__info['camera']['Radius']) / float(self.__info['camera']['fov_angle'])
            ...
            # calculate focal length
            focal_length = float(self.__info['camera'][
                                 'Radius']) / (2.0 * math.sin(math.radians(float(self.__info['camera']['fov_angle'])) / 2.0))
    ...
    def applyColourTable(self, colour_table):
        ...
        # apply colour table
        # PIL doesn't support 16bit images, so need to use own routine if "I"
        # mode image
        if self.__image.mode != "I":
    ...
    def binaryMask(self, fov_angle, inverted=False):
        ...
        if mode == "L" or mode == "RGB":
            ...
            # if the image is 16bit then cannot use the ImageChops module, so
            # use own multiplication routine
            if inverted:
                im_pix = numpy.asarray(self.__image)
                mask_pix = numpy.asarray(mask)

                new_image = Image.fromarray(
                    numpy.maximum(im_pix, 65535 - mask_pix))
    ...
    def centerImage(self):
        ...
        # get the bounding box of the image (box around non-zero parts of the
        # image)
        ...
        # the image is now pasted into a new image which encompasses the entire
        # (theoretical) circular field of view. This is done to allow PASKIL to
        # cope with images taken with non-circular fields of view.

        # create new square image with dimensions RadiusxRadius
        square_image = Image.new(
            self.__image.mode, (2 * r, 2 * r), color='black')

        # paste image into correct position in square image.
        width, height = new_image.size
        square_image.paste(new_image, (r - int(width / 2), r - int(height / 2),
                                       r - int(width / 2) + width, r - int(height / 2) + height))

        # create a new info dictionary
        new_info = self.getInfo()

        # update image center data
        new_info['camera']['x_center'] = int(
            (square_image.size[0] / 2) + 0.5)  # x coordinate of center
        new_info['camera']['y_center'] = int(
            (square_image.size[1] / 2) + 0.5)  # y coordinate of center
    ...
    def convertTo8bit(self):
        ...
        new_image = allskyImage(
            self.__image.convert("L"), self.__filename, info)
    ...
    def createQuicklook(self, size=(480, 640), timestamp="%a %b %d %Y, %H:%M:%S %Z", fontsize=16):
        ...
        if timestamp != None:
            ...
            im.paste(
                quicklook.getImage(), (0, 0, quicklook.getSize()[0], quicklook.getSize()[1]))
    ...
    def flatFieldCorrection(self, calibration):
        ...
        for x in range(new_image.size[0]):  # for x in range image width
            for y in range(new_image.size[1]):  # for y in range image height
                ...
                gradient = calibration.calibration_data[
                    int(angle) + 1] - calibration.calibration_data[int(angle)]
                correction = 1.0 / \
                    (calibration.calibration_data[
                     int(angle)] + (angle - float(int(angle))) * gradient)
    ...
    def getStrip(self, angle, strip_width):
        ...
        if (angle - float(self.__info['camera']['cam_rot'])) != 0.0:
            if self.__info['processing']['alignNorth'].count('NESW') != 0:
                im = self.__image.rotate(
                    angle - float(self.__info['camera']['cam_rot']))
        ...
        strip = numpy.zeros(
            (2 * radius, upper_x - lower_x, im_arr.shape[2]), im_arr.dtype)

        # the radius of the image may be larger than the image dimensions, in which
        # case we ensure that the strip is padded with zeros to the size of the
        # radius
    ...
    def keoSanityCheck(self, angle, fov, strip_width=5, colour=(255, 0, 0), fill=True):
        ...
        if fill:
            draw.rectangle(
                [lower_x, lower_y, upper_x + 1, upper_y + 1], outline=colour, fill=colour)
        else:
            draw.rectangle(
                [lower_x, lower_y, upper_x + 1, upper_y + 1], outline=colour)
    ...
    def _plot(self, subplot):
        ...
        if colour_table is not None:
            try:
                calib_factor = float(
                    self.__info['processing']['absoluteCalibration'])
            except KeyError:
                calib_factor = None
    ...
    def resize(self, size):
        ...
        if float(size[0]) / float(self.__image.size[0]) != float(size[1]) / float(self.__image.size[1]):
            ...
            size = (size[0], int(
                (float(size[0]) / float(self.__image.size[0])) * float(self.__image.size[1]) + 0.5))
        ...
        new_info = self.getInfo()
        new_info['camera']['x_center'] = int(
            int(new_info['camera']['x_center']) * scaling + 0.5)
        new_info['camera']['y_center'] = int(
            int(new_info['camera']['y_center']) * scaling + 0.5)
        new_info['camera']['Radius'] = int(
            int(new_info['camera']['Radius']) * scaling + 0.5)
    ...
    def save(self, filename, format="png"):
        ...
        if format == "png":  # save as png image
            ...
            if self.getMode() != "RGB":
                ...
                # images are indexed (x,y) whereas arrays are indexed (y,x)
                data_array = numpy.rot90(data_array.swapaxes(1, 0))

                # create a primary hdu to store the image
                primary_hdu = pyfits.PrimaryHDU(data_array)
                primary_hdu.verify('fix')

                # create primary hdu header, this is where all the data used by
                # PASKIL to re-load the file is stored
                primary_hdu_header = primary_hdu.header
                primary_hdu_header.update(
                    'PASKIL', 1, "File created using PASKIL")
                primary_hdu_header.update(
                    'PSKMODE', 'Intensities', "Image is greyscale")
                primary_hdu_header.update(
                    'PSKHEAD', 1, "HDU containing the header dictionary")
                primary_hdu_header.update(
                    'PSKCAM', 2, "HDU containing the camera dictionary")
                primary_hdu_header.update(
                    'PSKPRO', 3, "HDU containing the processing dictionary")
                primary_hdu_header.update(
                    'PSKEXIF', 4, "HDU containing the exif dictionary")

            else:  # image is RGB
                # convert the image object to an array
                r, g, b = self.__image.split()

                data_array = numpy.array([numpy.rot90(numpy.asarray(r).copy().swapaxes(1, 0)), numpy.rot90(
                    numpy.asarray(g).copy().swapaxes(1, 0)), numpy.rot90(numpy.asarray(b).copy().swapaxes(1, 0))])
                ...
                primary_hdu_header.update(
                    'PSKEXIF', 4, "HDU containing the exif dictionary")
                primary_hdu_header.add_comment(
                    'Sequence for NAXIS3   : RED, GREEN, BLUE')
            ...
            # Each header entry is stored in two variable length columns one
            ...
            header_cols_def = pyfits.ColDefs(
                [header_keys_col, header_values_col])
            camera_cols_def = pyfits.ColDefs(
                [camera_keys_col, camera_values_col])
            processing_cols_def = pyfits.ColDefs(
                [processing_keys_col, processing_values_col])
            exif_cols_def = pyfits.ColDefs([exif_keys_col, exif_values_col])

            # create one extension hdu for each image header library
            ...
            # create a hdu list to contain the primary hdu and the extension
            # hdus
            hdulist = pyfits.HDUList(
                [primary_hdu, header_hdu, camera_hdu, processing_hdu, exif_hdu])
            hdulist.verify('fix')
    ...
    def subtractBackgroundImage(self, background):
        ...
        if 'alignNorth' in self.__info['processing']:
            ...
            background = background.alignNorth(
                north=north, orientation=orientation)
        ...
        if self.__image.mode == 'L':
            new_image = ImageChops.subtract(
                self.__image, background.getImage())
        else:
    ...
    def xy2angle(self, x, y):
        ...
        if self.__info['camera']['lens_projection'] == 'equidistant':
            ...
            # calculate angle
            angle_radians = 2.0 * \
                math.asin(dist_from_center / (2.0 * focal_length))
