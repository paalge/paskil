...
#     lens_projection = "equidistant"
    if lens_projection == "equidistant":
        ...
        zenith_pixel = _generate_angle2pix_converter(
            keo_height, keo_fov_angle, lens_projection)(90.0)

        angle_from_zenith = lambda pix: 2.0 * \
            math.asin(math.fabs(zenith_pixel - pix) / (2.0 * focal_length))
...
def _calc_keo_width(times, start_time, end_time, strip_width, keo_type,
    ...
    if data_spacing == "AUTO":
        min_data_spacing, mean_data_spacing_secs = _estimate_data_spacing(
            times)
    else:
...
def _interpolateData(data_list, array, mode, colour_table, strip_width,
    ...
    # if there is only one data_list entry then return (no interpolation
    # possible)
    ...
    if mode == "L":
        ...
        cKeo.linear_interpolate(
            array[:, :, 0], data_list, strip_width, max_gap)
...
def _putData(image, keo_arr, strip_width, angle, keo_fov_angle, start_time,
    ...
    if fov_corrected_strip.shape[1] != height:
        ...
        strip_image = strip_image.resize(
            (height, fov_corrected_strip.shape[0]))
...
def _checkImages(images, mode=None, wavelength=None, colour_table=None,
    ...
    if calib_factor is None:
        try:
            calib_factor = images[
                0].getInfo()['processing']['absoluteCalibration']
        except KeyError:
            calib_factor = None
...
class keoTimeSlice(keoIntensitiesBase):
    ...
    def _plot(self, subplot):
        ...
        # convert the datetime objects into floating point numbers for use with
        # matplotlib
        time_data = []
...
class keogram:
    ...
    def _getVerticalStrip(self, time, strip_width):
        ...
        # mask out zero intensities
        masked_intensities = numpy.ma.array(
            intensities, mask=(intensities == 0))
    ...
    def roll(self, file_list):
        ...
        for im in images:
            ...
            time_str = im.getInfo()['header'][
                'Creation Time'].lstrip().rstrip()
            try:
        ...
        if self.__data_spacing == "AUTO":
            p2t = _generate_pix2time_converter(start_time, end_time,
                                               self.__width, self.__strip_width)
            data_spacing_secs = _estimate_data_spacing(
                [p2t(x) for x in new_data_points])[1]
    ...
    def save(self, filename):
        ...
        header['start_time'] = str(
            self.__start_time.strftime("%d %b %Y %H:%M:%S"))
